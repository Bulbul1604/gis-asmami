 ...
 <script>
    ...
    <?php foreach ($usaha as $value) : ?>
       L.marker([<?= $value->lang_lat ?>], {
          ...
          .bindPopup("<h6 style='letter-spacing: 1px;'><?= ucwords($value->nama_usaha) ?></h6><span><?= ucwords($value->alamat) ?> - <?= ucwords($value->kelurahan) ?> - <?= ucwords($value->kecamatan) ?></span><br /><br /><a href='<?= base_url('detail/' . $value->id) ?>' class='text-primary'>Detail UMKM</a>").
       addTo(map);
    <?php endforeach; ?>
 </script>
